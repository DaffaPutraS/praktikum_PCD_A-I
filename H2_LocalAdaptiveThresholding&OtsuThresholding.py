...
class ShowImage(QMainWindow):
    def __init__(self):
        ...
        # Add the connection for each new threshold action
        self.actionBinary.triggered.connect(lambda: self.globalThresholding(cv2.THRESH_BINARY))
        self.actionBinary_Invers.triggered.connect(lambda: self.globalThresholding(cv2.THRESH_BINARY_INV))
        self.actionTrunc.triggered.connect(lambda: self.globalThresholding(cv2.THRESH_TRUNC))
        self.actionTo_Zero.triggered.connect(lambda: self.globalThresholding(cv2.THRESH_TOZERO))
        self.actionTo_Zero_Invers.triggered.connect(lambda: self.globalThresholding(cv2.THRESH_TOZERO_INV))

        # Connect adaptive thresholding actions
        self.actionMean_Thresholding.triggered.connect(self.adaptiveMeanThresholding)
        self.actionGaussian_Thresholding.triggered.connect(self.adaptiveGaussianThresholding)

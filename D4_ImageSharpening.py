        ...
        for j in range(3, w - 3):
            ...
            for k in range(-3, 4):
                for l in range(-3, 4):
                    ...
                    neighbors.append(a)
            neighbors.sort()
            median = neighbors[24]  # Median value in the sorted list
            img_out[i, j] = median
    return img_out
...
def convert_and_median_filter():
    global original_image
    grayscale_image = cv2.cvtColor(original_image, cv2.COLOR_BGR2GRAY)
    median_filtered_image = median_filter(grayscale_image)
...
median_button = Button(root, text="Median Filter", command=convert_and_median_filter)
median_button.grid(row=4, column=0, columnspan=3)
